 ...
 * @LastEditors: wch
 * @LastEditTime: 2023-08-15 14:34:06
 */
...
public class IndicatorController {
    ...
    public ResultUtils getFavourIndicators(@RequestHeader("Authorization") String token, @RequestBody IndicatorQuery indicatorQuery) {
        if (indicatorQuery.getNeedPage() == null || (indicatorQuery.getNeedPage() == true
                && (indicatorQuery.getPage() == null || indicatorQuery.getPageSize() == null)))
        ...
        try {
            if (indicatorQuery.getNeedPage() == true)
                return ResultUtils.ok().data("indicators",
                        indicatorService.getUserFavourIndicatorsWithPage(user, indicatorQuery));
            else
     ...
     */
    @PostMapping("/create")
    public ResultUtils getIndicatorsByCreator(@RequestHeader("Authorization") String token,
            @RequestBody IndicatorQuery indicatorQuery) {
        if (indicatorQuery.getNeedPage() == null || (indicatorQuery.getNeedPage() == true
                && (indicatorQuery.getPage() == null || indicatorQuery.getPageSize() == null)))
            return ResultUtils.error().message("缺少分页参数");
        indicatorQuery.setKeyword(null);
        ...
        try {
            if (indicatorQuery.getNeedPage() == true)
                return ResultUtils.ok().data("indicators",
                        indicatorService.getIndicatorsWithPage(user, indicatorQuery));
            else
     ...
     */
    @PostMapping("/find")
    public ResultUtils findIndicators(@RequestHeader("Authorization") String token,
            @RequestBody IndicatorQuery indicatorQuery) {
        if (indicatorQuery.getNeedPage() == null || (indicatorQuery.getNeedPage() == true
                && (indicatorQuery.getPage() == null || indicatorQuery.getPageSize() == null)))
            return ResultUtils.error().message("缺少分页参数");
    ...
    public ResultUtils getIndicatorTree(@RequestHeader("Authorization") String token,
    ...
    public ResultUtils updateIndicatorState(@RequestHeader("Authorization") String token, @RequestParam int newState,
            @RequestParam String indicatorId) {
        User user = userService.findUserByToken(token);
    ...
    public ResultUtils deleteIndicator(@RequestHeader("Authorization") String token, @RequestParam String indicatorId) {
        if (indicatorId == null || indicatorId.length() == 0)
            return ResultUtils.error().message("缺少指标id");
        Indicator indicator = indicatorService.getById(indicatorId);
        if (indicator == null)
            return ResultUtils.error().message("指标不存在");
        try{
            indicatorService.removeIndicator(indicator);
            return ResultUtils.ok().message("删除指标成功");
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
            return ResultUtils.error().message(e.getMessage());
        } catch (Exception e){
            e.printStackTrace();
            return ResultUtils.error().message("删除指标失败");
        }
    }
}
