...
public class ModifierServiceImpl extends ServiceImpl<ModifierMapper, Modifier> implements ModifierService {
    ...
    public List<Modifier> getModifiers(User user, WordsQuery wordsQuery) {
        ...
        return modifierMapper.selectModifiers(user, queryWrapper);
    }
    ...
    public Boolean verifyModifier(Modifier modifier) throws IllegalArgumentException{
        ...
        if (modifier.getParentModifierId() != null && getById(modifier.getParentModifierId()).getParentModifierId() != null)
    ...
    /**
     * @description: 删除修饰词值
     * @param {String} value 修饰词值名称
     * @param {int} parentId 修饰词键id
    ...
    private QueryWrapper<Modifier> getQueryWrapper(WordsQuery wordsQuery) {
        ...
        if (wordsQuery.getSort() != null && wordsQuery.getKeyword() != null) {
            queryWrapper.and(new Consumer<QueryWrapper<Modifier>>() {
                @Override
                public void accept(QueryWrapper<Modifier> modifierQueryWrapper) {
                    modifierQueryWrapper.like(wordsQuery.getSort().equals("all") || wordsQuery.getSort().equals("modifierName"), "modifier_name", wordsQuery.getKeyword());
                    modifierQueryWrapper.or();
                    modifierQueryWrapper.like(wordsQuery.getSort().equals("all") || wordsQuery.getSort().equals("creator"), "username", wordsQuery.getKeyword());
                }
            });
        }
        return queryWrapper;
     ...
     * @param {modifier} modifier 修饰词键
     * @param {Map<Integer, List<Modifier>>} modifierMap 修饰词键-修饰词值映射
     * @param {Boolean} allowParent 是否允许父节点被选中
     * @return {*}
     * @author: wch
     */    
    private Map<String, Object> createAllModifierNode(Modifier modifier, Map<Integer, List<Modifier>> modifierMap, Boolean allowParent) {
