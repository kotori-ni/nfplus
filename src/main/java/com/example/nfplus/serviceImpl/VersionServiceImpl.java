 ...
 * @LastEditors: wch
 * @LastEditTime: 2023-08-15 14:19:16
 */
...
public class VersionServiceImpl extends ServiceImpl<VersionMapper, Version> implements VersionService {
     ...
     * @param {Indicator} oldIndicator 修改前的指标
     * @param {Indicator} newIndicator 修改后的指标
    ...
    public Version getVersionInfo(Indicator oldIndicator, Indicator newIndicator) {
        ...
        if (!newIndicator.getDomainId().equals(oldIndicator.getDomainId()))
            operation += "指标域由 " + domainMapper.selectById(oldIndicator.getDomainId()).getDomainName() + " 改为 " + domainMapper.selectById(newIndicator.getDomainId()).getDomainName() + " \n";
        ...
        if (!newIndicator.getBusinessCaliberLeader().equals(oldIndicator.getBusinessCaliberLeader()))
            operation += "业务口径负责人由 " + oldIndicator.getBusinessCaliberLeader() + " 改为 " + newIndicator.getBusinessCaliberLeader() + " \n";
        ...
        if ((newIndicator.getAffiliatedReportLinks() != null && oldIndicator.getAffiliatedReportLinks() != null)
            ...
            operation += "关联报表链接由 " + oldIndicator.getAffiliatedReportLinks() + " 改为 " + newIndicator.getAffiliatedReportLinks() + " \n";
        ...
        if ((newIndicator.getAnalyzableDimensions() != null && oldIndicator.getAnalyzableDimensions() != null)
            ...
            operation += "可分析维度由 " + oldIndicator.getAnalyzableDimensions() + " 改为 " + newIndicator.getAnalyzableDimensions() + " \n";
        ...
        if ((newIndicator.getTechnicalCaliberLeader() != null && oldIndicator.getTechnicalCaliberLeader() != null)
            ...
            operation += "技术口径负责人由 " + oldIndicator.getTechnicalCaliberLeader() + " 改为 " + newIndicator.getTechnicalCaliberLeader() + " \n";
        ...
        if ((newIndicator.getRealtimeTechnicalCaliber() != null && oldIndicator.getRealtimeTechnicalCaliber() != null)
            ...
            operation += "实时技术口径由 " + oldIndicator.getRealtimeTechnicalCaliber() + " 改为 " + newIndicator.getRealtimeTechnicalCaliber() + " \n";
        else if (oldIndicator.getRealtimeTechnicalCaliber() == null && newIndicator.getRealtimeTechnicalCaliber() != null)
        ...
        if ((newIndicator.getCompetentAuthoritie() != null && oldIndicator.getCompetentAuthoritie() != null)
            ...
            operation += "主管部门由 " + oldIndicator.getCompetentAuthoritie() + " 改为 " + newIndicator.getCompetentAuthoritie() + " \n";
        ...
        if (newIndicator.getIndicatorType() == Indicator.TYPE_MODIFIER && !oldIndicator.getModifiers().equals(newIndicator.getModifiers())) {
            operation += "修饰词由 ";
        ...
        if (newIndicator.getIndicatorType() == Indicator.TYPE_MODIFIER && newIndicator.getTimeCycleId() != null
                && oldIndicator.getTimeCycleId() != null && !oldIndicator.getTimeCycleId().equals(newIndicator.getTimeCycleId()))
            operation += "时间周期由 " + timeCycleService.getById(oldIndicator.getTimeCycleId()).getTimeCycleName() + " 改为 " + timeCycleService.getById(newIndicator.getTimeCycleId()).getTimeCycleName() + " \n";
        else if (newIndicator.getIndicatorType() == Indicator.TYPE_MODIFIER && newIndicator.getTimeCycleId() != null && oldIndicator.getTimeCycleId() == null)
            operation += "时间周期新增为 " + timeCycleService.getById(newIndicator.getTimeCycleId()).getTimeCycleName() + " \n";
        else if (newIndicator.getIndicatorType() == Indicator.TYPE_MODIFIER && newIndicator.getTimeCycleId() == null && oldIndicator.getTimeCycleId() != null)
            operation += "时间周期由 " + timeCycleService.getById(oldIndicator.getTimeCycleId()).getTimeCycleName() + " 改为 null \n";

        if (newIndicator.getIndicatorType() == Indicator.TYPE_COMPOSITE && !oldIndicator.getCompositeds().equals(newIndicator.getCompositeds())) {
            operation += "依赖的其他指标由 ";
